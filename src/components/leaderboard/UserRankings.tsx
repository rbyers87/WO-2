...
console.log('Fetched data:', data);
        ...
        const userStats = data.reduce((acc: Record<string, UserRanking>, log) => {
          ...
          acc[profile.id].total_score += log.total || 0; // Use the `total` column from workout_logs
          acc[profile.id].daily_score += log.total || 0; // Use the `total` column from workout_logs
          return acc;
        }, {});

        // Sort users by daily_score in descending order
        const sortedRankings = Object.values(userStats).sort((a, b) => b.daily_score - a.daily_score).slice(0, 10);
